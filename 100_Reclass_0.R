# 11_post_processing_models.R 
#
# Two post-classification correction models, scored per county vs. Census:
#   - MMU: patch removal + modal fill over a grid of parameters (mmu_model_grid)
...
#
# best_model_per_county selection happens in the next code after we combine all parts.
#
# IMPORTANT: 
# WHICHPART: set manually below. 0 = test (N_TEST_COUNTIES counties).
# N_PARTS = split of county_lookup (~3000 counties / N_PARTS each).
# We use N_PARTS to allow run on multiple nodes. (which still take some times)
# On anubis (between 28 and 40 nodes, with 250go RAM and we take 6h per 300 counties)
#
# To ensure resumability, each county's MMU results and each (state, county)
# pair's CSB result is written to its OWN small .rds file the moment it finishes:
...
# that GEOID's .rds already exists. Ff so, they skip it immediately meaning 
# no raster read nor computation. 

# This allow us to just resubmit the same .slurm file if it crashes. 
# Also, the final combining step only relies on every .rds file in 
# mmu_by_county/ and csb_by_county/.
#
...
# per-county checkpoint directories = where the .rds per GEOID are saved.
...
# function to run one MMU model over a category raster
...
###
# Function to run the CSB field-modal model over a category raster
...
###
# Function to save a diagnostic plot for one model's output raster
...
###
# Function for confidence of reclassified pixels vs. all cropland (shared by
# every model type. One row per (GEOID, model)
...
###
# Function for results and quality for each model
...
###
# Function to process MMU for one county so we can parallelee at county level.
# It checks mmu_county_path(geoid) first so we can resume in case of crash 
# Once the county-full grid is done, writes the county's results/quality to that
...
###
# Function for the CSB field-modal model for a STATE. Because reading the state's
# CSB polygons is very long so it avoid doing it for each county.
# Then loops every county in that state. Hence, CSB is parrallelized at state level
# but since it's only the reading of the initial pologyons which take times it's ok.

# RESUMABLE: county's result checkpointed in csb_county_path(geoid) using same logic
# as the MMU function above
...
process_state_csb <- function(statefp, geoids_in_state, baseline_all, counties_sf,
  ...
  # this state (within this part) has already a checkpoint
...
###
#Finally, reads EVERY .rds checkpoint file currently on disk for this part.
